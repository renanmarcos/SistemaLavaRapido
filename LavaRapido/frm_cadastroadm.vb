    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Else
            If StrComp(txt_senha.Text, txt_repetir.Text, vbBinaryCompare) = 0 Then
                ...
                sql = "SELECT * FROM tb_adm WHERE rg='" & txt_rg.Text & "' OR usuario='" & txt_usuario.Text & "'"
                rs = db.Execute(sql)

                If rs.EOF = True Then
                    Try
                        sql = "INSERT INTO tb_adm (rg, usuario, senha, cpf, datanasc, foneres, cel, email," &
                                  "pergunta_secreta, resposta_secreta, tipo_conta, status_conta, n_tentativas) VALUES ('" & txt_rg.Text & "', '" _
                                  & txt_usuario.Text & "', '" & txt_senha.Text & "', '" _
                                  & txt_cpf.Text & "', '" & txt_dtnasc.Text & "', '" & txt_foneres.Text & "', '" _
                                  & txt_cel.Text & "', '" & txt_email.Text & "', '" & cmb_perguntas.Text & "', '" _
                                  & txt_resposta.Text & "', '" & cmb_tipo.Text & "', 'ativa', 3)"
                        db.Execute(sql)
